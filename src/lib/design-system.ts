...
// Safe proxy that never throws on property access
function safeDs(obj: Record<string, any>): Record<string, any> {
  return new Proxy(obj, {
    get(target, prop) {
      if (typeof prop === "symbol") return undefined;
      const val = target[prop];
      if (val === undefined) {
        console.warn(`[design-system] Missing property: ds.${String(prop)}`);
        return {};
      }
