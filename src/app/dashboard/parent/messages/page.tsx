  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] overflow-hidden animate-fade-in">
      <div className={cn("w-full sm:w-[300px] border-r border-white/60 bg-white flex flex-col flex-shrink-0", activeConv && !showSidebar ? "hidden sm:flex" : "flex")}>
        ...
        {showNewConv && (
          <div className="p-4 border-b border-white/60 bg-secondary/5">
            ...
            {members.length === 0 ? <p className="text-xs text-text-muted">No contacts available.</p> : (
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {members.map((m: any) => (
                  <button key={m.id} onClick={() => startConversation(m.id)} className="w-full flex items-center gap-3 p-3 rounded-xl bg-white border border-white/60 hover:border-secondary/30 transition-all text-left">
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            ...
            <button key={conv.id} onClick={() => { setActiveConv(conv); setShowSidebar(false); }} className={cn("w-full p-4 border-b border-white/60/50 flex items-center gap-3 transition-all text-left hover:bg-secondary/5", activeConv?.id === conv.id && "bg-secondary/5 border-l-4 border-l-secondary")}>
          ...
          <>
              ...
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={cn("flex", isMine ? "justify-end" : "justify-start")}>
                    <div className={cn("max-w-[75%] px-4 py-2.5 rounded-2xl text-sm", isMine ? "bg-secondary text-white rounded-br-md" : "bg-white border border-white/60 text-text rounded-bl-md shadow-sm")}>
                      <p>{msg.body}</p>
            ...
            <div className="p-4 border-t border-white/60 bg-white">
              <div className="flex items-center gap-2">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} placeholder="Type a message..." className="flex-1 px-4 py-3 rounded-2xl border border-white/60 bg-bg-main text-sm text-text placeholder:text-text-muted focus:outline-none focus:border-secondary/50 focus:ring-2 focus:ring-secondary/10" />
