      ...
      <div className={cn("w-full sm:w-[300px] border-r border-border-soft bg-white flex flex-col flex-shrink-0", activeConv && !showSidebar ? "hidden sm:flex" : "flex")}>
          ...
          <div className="p-4 border-b border-border-soft bg-primary/5">
            ...
            {members.length === 0 ? <p className="text-xs text-text-muted">No parents available.</p> : (
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {members.map((m: any) => (
                  <button key={m.id} onClick={() => startConversation(m.id)} className="w-full flex items-center gap-3 p-3 rounded-xl bg-white border border-border-soft hover:border-primary/30 transition-all text-left">
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            ...
            <button key={conv.id} onClick={() => { setActiveConv(conv); setShowSidebar(false); }} className={cn("w-full p-4 border-b border-border-soft/50 flex items-center gap-3 transition-all text-left hover:bg-primary/5", activeConv?.id === conv.id && "bg-primary/5 border-l-4 border-l-primary")}>
              ...
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={cn("flex", isMine ? "justify-end" : "justify-start")}>
                    <div className={cn("max-w-[75%] px-4 py-2.5 rounded-2xl text-sm", isMine ? "bg-primary text-white rounded-br-md" : "bg-white border border-border-soft text-text rounded-bl-md shadow-sm")}>
                      <p>{msg.body}</p>
