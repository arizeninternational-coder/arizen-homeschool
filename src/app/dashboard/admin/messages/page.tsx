...
export default function AdminMessagesPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden animate-fade-in">
      <div className={cn("w-full sm:w-[300px] border-r border-white/60 bg-white flex flex-col flex-shrink-0", activeConv && !showSidebar ? "hidden sm:flex" : "flex")}>
        <div className="p-4 border-b border-white/60">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-extrabold text-text">Messages</h2>
            <button onClick={() => setShowNewConv(true)} className="w-8 h-8 rounded-xl bg-primary text-white flex items-center justify-center hover:bg-primary-dark transition-colors"><Plus size={16} /></button>
          </div>
          <p className="text-xs text-text-muted">Message parents of your students.</p>
        </div>
        {showNewConv && (
          <div className="p-4 border-b border-white/60 bg-primary/5">
            <h3 className="text-sm font-bold text-text mb-3">New conversation</h3>
            {members.length === 0 ? <p className="text-xs text-text-muted">No parents available.</p> : (
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {members.map((m: any) => (
                  <button key={m.id} onClick={() => startConversation(m.id)} className="w-full flex items-center gap-3 p-3 rounded-xl bg-white border border-white/60 hover:border-primary/30 transition-all text-left">
                    <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-secondary to-secondary-dark flex items-center justify-center text-white text-sm font-bold">{(m.name || "U").charAt(0)}</div>
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            <div className="p-8 text-center"><MessageCircle className="w-10 h-10 text-text-muted mx-auto mb-3 opacity-50" /><p className="text-sm font-bold text-text-muted mb-1">No conversations yet</p><p className="text-xs text-text-muted">Tap + to message a parent.</p></div>
          ) : conversations.map((conv) => (
            <button key={conv.id} onClick={() => { setActiveConv(conv); setShowSidebar(false); }} className={cn("w-full p-4 border-b border-white/60/50 flex items-center gap-3 transition-all text-left hover:bg-primary/5", activeConv?.id === conv.id && "bg-primary/5 border-l-4 border-l-primary")}>
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary/20 to-accent-purple/20 flex items-center justify-center text-primary text-sm font-bold flex-shrink-0">{getName(conv).charAt(0)}</div>
      ...
      <div className={cn("flex-1 flex flex-col", !activeConv && !showSidebar ? "hidden sm:flex" : "flex")}>
        {activeConv ? (
          <>
            <div className="p-4 border-b border-white/60 bg-white flex items-center gap-3">
              ...
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary/20 to-accent-purple/20 flex items-center justify-center text-primary text-sm font-bold">{getName(activeConv).charAt(0)}</div>
              ...
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={cn("flex", isMine ? "justify-end" : "justify-start")}>
                    <div className={cn("max-w-[75%] px-4 py-2.5 rounded-2xl text-sm", isMine ? "bg-primary text-white rounded-br-md" : "bg-white border border-white/60 text-text rounded-bl-md shadow-sm")}>
                      <p>{msg.body}</p>
            ...
            <div className="p-4 border-t border-white/60 bg-white">
              <div className="flex items-center gap-2">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} placeholder="Type a message..." className="flex-1 px-4 py-3 rounded-2xl border border-white/60 bg-bg-main text-sm text-text placeholder:text-text-muted focus:outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10" />
                <button onClick={sendMessage} disabled={!input.trim() || sending} className="w-11 h-11 rounded-2xl bg-primary text-white flex items-center justify-center hover:bg-primary-dark transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0">
