...
async function main() {
  ...
  if (!themes?.length) { console.log('No themes'); process.exit(1); }
  
  let grandTotal = 0, grandDraft = 0, grandApproved = 0, grandNone = 0;
  ...
  for (const theme of themes) {
    ...
    grandDraft += withDraft;
    grandApproved += withApproved;
    grandNone += withNone;
    
    console.log(theme.title + ' (' + theme.slug + '): ' + all.length + ' lessons | Draft: ' + withDraft + ' | Approved: ' + withApproved + ' | None: ' + withNone);
    console.log('  Status:', JSON.stringify(statusCounts));
  }
  
  console.log('\n=== GRADE 2 TOTAL ===');
  console.log('Total lessons: ' + grandTotal);
  console.log('With draft journeys: ' + grandDraft);
  console.log('With approved journeys: ' + grandApproved);
  console.log('Without journeys: ' + grandNone);
}
